...
// The API resource's name in AppHost.cs — `${app-name}-server`, so it changes with app-name.
// Aspire derives the injected env var names from it, so keep the two in sync; renaming the
// resource without updating this is what previously left the dev proxy target undefined and
// made every /api call fail with a 502.
const SERVER_RESOURCE = 'aspire-server';

// The /api proxy is a DEV-SERVER concern only. The published image is static assets served by
// YARP, which does its own /api proxying (PublishAsStaticWebsite in the AppHost) — and that
// image is built in a container with no Aspire env vars, so requiring them here at build time
// is what broke `npm run build` inside the Docker build.
